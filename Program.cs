...
public class Program {
	public static void Main(string[] args) {
		...
		candidates.Add(new Candidate("LotR:Fellowship", Color.green));
		candidates.Add(new Candidate("DarkKnight"));
		candidates.Add(new Candidate("Spiderman", Color.red));
		candidates.Add(new Candidate("Matrix"));
		candidates.Add(new Candidate("Terminator2"));
		candidates.Add(new Candidate("JurassicPark"));
		candidates.Add(new Candidate("Tron:Legacy", Color.cyan));
		candidates.Add(new Candidate("BladeRunner", Color.darkBlue));
		candidates.Add(new Candidate("BackToTheFuture"));
		candidates.Add(new Candidate("Aliens", Color.green));
		candidates.Add(new Candidate("SW:NewHope"));
		...
		Print.Pause();
		Print.Clear();
		Candidate.InstantVoteCalc(votes, out _, out _);
		votes.Sort();
		Print.ShowAllBallotsSortedByCandidate(votes, candidates);
		Print.Pause();
		Print.Clear();
			...
			if (allData != null) {
				...
				for (int e = 0; e < allData.Count; ++e) {
					...
					for (int i = 0; i < allStates.Count; ++i) {
						...
						VoteState? next_vState = i < election.voteStates.Count-1 ? election.voteStates[i + 1] : null;
